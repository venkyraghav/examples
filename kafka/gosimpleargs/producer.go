...
func produce(ctx context.Context, cmd *cli.Command) error {
	sigchan := make(chan os.Signal, 1)
	signal.Notify(sigchan, syscall.SIGINT, syscall.SIGTERM)
	kafkaCommand := NewKafkaCommand()

	if err := kafkaCommand.validate(ctx, cmd, Produce); err != nil {
		return err
	}

	configMap, err := kafkaCommand.getClientConfig()
	if err != nil {
		return nil
	}

	// infoLogger.Printf("ConfigMap %v", &configMap)
	...
	msgcnt := int64(0)
	for msgcnt < kafkaCommand.RecordCount {
		value := fmt.Sprintf("Producer example, message #%d", msgcnt)
		if err := p.Produce(&kafka.Message{
			TopicPartition: kafka.TopicPartition{Topic: &kafkaCommand.Topic, Partition: kafka.PartitionAny},
			Value:          []byte(value),
			...
			if kafkaCommand.TPS > 0 {
				time.Sleep(time.Millisecond * time.Duration(1000/kafkaCommand.TPS))
			}
		}
		msgcnt++
	}
