...
public class DatagenProducer extends ProducerBase {
    ...
    public DatagenProducer(ProducerCommand command) {
        super(command);
    ...
    protected void doProcess(Properties props) {
        try (final Producer<Object, Object> producer = new KafkaProducer<>(props)) {
            ...
            for (long outer = 0; outer < command.getPayloadCount(); outer++) {
                for (long i = 0; i < numMessages; i++) {
                    ProducerRecord<Object, Object> record = new ProducerRecord<>(command.getTopic(), 
                            getKey(command.getKeyFormat(), 10), 
                            getValue(command.getValueFormat(), command.getPayloadSize()));
                    producer.send(
                            record,
                            (metadata, ex) -> {
                                if (ex != null) {
                ...
                if (log.isInfoEnabled()) {log.info("{} events were produced to topic {}", numMessages, command.getTopic());}
